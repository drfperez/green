...
def calcular_percentatge_verd(frame):
    ...
    rang_verd_min = np.array([35, 100, 100])    # Valores mínimos para el verde en HSV
    ...
    mask = cv2.inRange(frame_hsv, rang_verd_min, rang_verd_max)
...
while True:
    try:
        ...
        # Salir del bucle si no hay más frames disponibles
        if not ret:
            break
